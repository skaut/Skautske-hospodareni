...
class Accountancy_ExportPresenter extends Accountancy_BasePresenter {
    ...
    public function renderParticipants($aid){
        ...
        $template->info = $info;
        $template->list = $list;
        $template->setFile(dirname(__FILE__) . '/../templates/Export/ex.participants.latte');
        $this->context->participantService->makePdf($template, Strings::webalize($info->DisplayName) . "_ucastnici.pdf");
        $this->terminate();
    }
