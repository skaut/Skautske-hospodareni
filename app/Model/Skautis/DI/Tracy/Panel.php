...
namespace App\Model\Skautis\DI\Tracy;
...
/**
 * Tracy panel se seznamem Skautis dotazů.
 *
 * Inlinováno z opuštěného balíčku skautis/nette.
 */
final class Panel implements Tracy\IBarPanel
{
    ...
    /**
     * Zaregistruje naslouchání událostem WebService přes WsdlManager.
     */
    public function register(WsdlManager $wsdlManager): void
    {
        // WsdlManager::addWebServiceListener chybně dokumentuje $eventName jako string, ale konstanty
        // WebService::EVENT_* jsou int a runtime je porovnává striktně – int je tedy správně.
     ...
     */
    private function prepareTrace(array $trace): string
    {
        $s = '';
        $cnt = 0;
        foreach ($trace as $f) {
    ...
    private function dump(mixed $object): string
    {
        return Tracy\Debugger::dump($object, true);
    }

    private function formatToggle(string $name, ?string $rel = null): string
    {
