...
use App\Components\Factories\Cashbook\IChitScanControlFactory;
use App\Components\Factories\Cashbook\IInvertChitDialogFactory;
use App\Components\Factories\Cashbook\IMoveChitsDialogFactory;
use App\Components\Factories\Cashbook\IPrefixControlFactory;
...
class ChitListControl extends BaseControl
{
    ...
    public function handleRemove(int $chitId): void
    {
        ...
        try {
            ...
            $this->flashMessage('Nelze smazat zamčený paragon', 'danger');
    ...
    public function handleShowLocked(): void
    {
        $this->getPresenter()->redirect(
            ':Unit:Chit:default',
            [
                'unitId' => (int) $this->getPresenter()->getParameter('unitId'),
                'year' => $this->getPresenter()->getParameter('year'),
                'onlyUnlocked' => 0,
            ],
        );
    }
    ...
    public function handleGenerateNumbers(string $paymentMethod): void
    {
        try {
            ...
            $this->getPresenter()->flashMessage('Nelze generovat čísla, když čísla dokladů jsou nečíselné!', 'danger');
        } catch (MaxChitNumberNotFound) {
            $this->getPresenter()->flashMessage('Nepodařilo se určit poslední poslední paragon, od kterého by se pokračovalo s číslováním.', 'danger');
        }
    ...
    protected function createComponentFormMass(): BaseForm
    {
        ...
        $form->onSuccess[] = function (BaseForm $form) use ($printButton, $exportButton, $moveChitsButton): void {
            ...
            if ($exportButton->isSubmittedBy()) {
                $this->redirectToExport(':Unit:CashbookExport:exportChits', $chitIds);
